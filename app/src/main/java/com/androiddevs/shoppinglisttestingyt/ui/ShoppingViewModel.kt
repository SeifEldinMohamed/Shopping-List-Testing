...
class ShoppingViewModel @ViewModelInject constructor(
    ...
    private val _images = MutableLiveData<Event<Resource<ApiResponse>>>()
    val images: LiveData<Event<Resource<ApiResponse>>> = _images

    private val _curImageUrl = MutableLiveData<String>()
    val curImageUrl: LiveData<String> = _curImageUrl

    private val _insertShoppingItemStatus = MutableLiveData<Event<Resource<ShoppingEntity>>>()
    val insertShoppingItemStatus: LiveData<Event<Resource<ShoppingEntity>>> = _insertShoppingItemStatus

    fun setCurImageUrl(url: String) {
        _curImageUrl.postValue(url)
    }

    fun deleteShoppingItem(shoppingItem: ShoppingEntity) = viewModelScope.launch {
        repository.deleteShoppingItem(shoppingItem)
    }

    fun insertShoppingItemIntoDb(shoppingItem: ShoppingEntity) = viewModelScope.launch {
    ...
    fun insertShoppingItem(name: String, amountString: String, priceString: String) {
        ...
        val shoppingItem = ShoppingEntity(1,name, amount, priceString.toFloat(), _curImageUrl.value ?: "")
